<template>
  <div class="picture-preview-com all0">
    <div class="img-wrapper" v-if="!error">
      <img
              v-show="!imgIsLoading"
              alt="图片地址"
              class="img-content"
              ...
              @load="load">
      <div v-if="imgIsLoading" style="width: 400px" class="img-loading-tips"></div>
    </div>
    ...
    <div class="img-error img-wrapper" v-if="error">
      图片加载失败
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    created() {
      ...
      //console.log("当前图片为：", this.img_url)
